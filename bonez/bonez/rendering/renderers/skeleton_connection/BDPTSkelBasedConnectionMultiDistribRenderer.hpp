...
class BDPTSkelBasedConnectionMultiDistribRenderer: public TileProcessingRenderer {
private:
    ...
    Vec3f evalLightVertexContrib(const PathVertex& eyeVertex, std::size_t lightPathDepth,
                                 const Sample1u& lightVertexSample) const;

    GraphNodeIndex skeletonMapping(const PathVertex& eyeVertex) const;
